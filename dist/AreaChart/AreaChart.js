...
_defineProperty(AreaChart, "propTypes", {
  ...
  curveType: _propTypes["default"].func,
  ...
  xDomain: _propTypes["default"].arrayOf([_propTypes["default"].number, _propTypes["default"].number]),
  yDomain: _propTypes["default"].arrayOf([_propTypes["default"].number, _propTypes["default"].number]),
  xRange: _propTypes["default"].arrayOf([_propTypes["default"].number, _propTypes["default"].number]),
  yRange: _propTypes["default"].arrayOf([_propTypes["default"].number, _propTypes["default"].number]),
...
var D3AreaChart = function () {
  ...
  _createClass(D3AreaChart, [{
    ...
    value: function render(data, attr) {
      ...
      var area0 = d3.area().x(function (i) {
        return xScale(x[i]);
      }).y0(height - marginBottom).y(height - marginBottom),
          area = d3.area().defined(function (i) {
        return defined[i];
      }).curve(curveType).x(function (i) {
        return xScale(x[i]);
      ...
      areaPath.append("path").datum(I).attr("fill", areaColor).style("opacity", areaOpacity).attr("d", area);
      var linePath = svg.append("g");

      if (enableLinePath) {
        linePath.append("path").datum(I).attr("fill", "none").attr("stroke", strokeColor).attr("stroke-width", strokeWidth).attr("stroke-linecap", strokeLinecap).attr("stroke-linejoin", strokeLinejoin).attr("stroke-opacity", strokeOpacity).attr("d", line);
      }
      ...
      if (enableLineNode) {
        lineNode.selectAll("circle").data(I).join("circle").attr("cx", function (i) {
          ...
          return yScale(y[i]);
        }).attr("r", lineNodeRadius).attr("fill", "white").attr("stroke", strokeColor).attr("stroke-width", strokeWidth);
      }

      if (enableAnimation) {
        areaPath.selectAll("path").attr("fill", "rgba(0, 0, 0, 0)").attr("d", area0).transition().attr("fill", areaColor).attr("d", area).duration(animationTime);

        if (enableLinePath) {
          var pathLenth = linePath.select("path").node().getTotalLength();
          linePath.attr("stroke-dasharray", pathLenth + " " + pathLenth).attr("stroke-dashoffset", pathLenth).transition().ease(d3.easeLinear).attr("stroke-dashoffset", 0).duration(animationTime).delay(animationTime);
        }
