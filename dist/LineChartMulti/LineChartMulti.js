...
exports.LineChartMulti = LineChartMulti;
...
_defineProperty(LineChartMulti, "propTypes", {
  ...
  strokeColor: _propTypes["default"].arrayOf(_propTypes["default"].string),
...
var D3LineChartMulti = function () {
  function D3LineChartMulti(element) {
    _classCallCheck(this, D3LineChartMulti);

    this.svg = d3.select(element);
  }

  _createClass(D3LineChartMulti, [{
    key: "render",
    value: function render(data, attr) {
      ...
      if (strokeColor === undefined) strokeColor = d3.quantize(function (t) {
        return d3.interpolateSpectral(t * 0.8 + 0.1);
      }, keysOfGroups.length);
      var strokeColorScale = d3.scaleOrdinal(keysOfGroups, strokeColor);
      ...
      function selectOne(_, d) {
        ...
        groupData.map(function (data) {
          if (!(data.group === d) && !(d === "all")) {
            linePath.select("._" + data.group).transition().attr("stroke-width", 0).duration(500);
            lineNode.selectAll("._" + data.group).transition().attr("r", 0).duration(500);
          } else if (data.group === d) {
            linePath.select("._" + data.group).transition().attr("stroke-width", strokeWidth).duration(500);
            lineNode.selectAll("._" + data.group).transition().attr("r", lineNodeRadius).duration(500);
          }
        });
      }
      ...
      function selectAll() {
        selectedOne = false;
        linePath.selectAll(".all").data(groupData).transition().attr("stroke-width", strokeWidth).duration(500);
        lineNode.selectAll(".all").transition().attr("r", lineNodeRadius).duration(500);
      }
    }
  }]);

  return D3LineChartMulti;
}();
