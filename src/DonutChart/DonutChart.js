...
class DonutChart extends Component {
  ...
  componentDidMount() {
    ...
    const element = this.element, donut = new D3DonutChart(element);
...
class D3DonutChart {
  ...
  render(data, attr) {
    ...
    let {
      ...
      padAngle, chartTitleText, animationTime, enableAnimation, textSize, pieLabelStrokeColor, pieLabelStrokeWidth,
    ...
    function pieLabelLine(data) {
      pieLabel
        ...
        .join("polyline")
          .transition()
          .attr("class", d => "all pieLabelLine" + name[d.data])
          .attr("stroke", pieLabelStrokeColor)
          .style("fill", "none")
    ...
    function showTooltip(_, d) {
      ...
      tooltip.attr("transform", `translate(${p[0] + pieCentroid[0]}, ${p[1] + pieCentroid[1]})`);
    ...
    if (enableLegend) {
      ...
      legend
        ...
        .join("circle")
          ...
          .attr("class", i => "all legend_" + name[i])
          .attr("cx", 0)
      ...
      function pieTransformAnimation(animationTime) {
        pie
          ...
          .attrTween("d", d => {
            const
              Start = d3.interpolate(currentDivData[d.data].startAngle, d.startAngle),
      ...
      function selectOne(_, i) {
        ...
        if (pieDefined[i] === false)
          legend.selectAll(".legend_" + name[i]).style("opacity", 0.2);
        else
          legend.selectAll(".legend_" + name[i]).style("opacity", 1);
        const newDivData = d3.pie().padAngle(padAngle).sort(null).value(i => value[i])(I.slice(0, -1).filter(i => pieDefined[i] === true ));
      ...
      // ******* here has some bug (fixed), but no reason -> divData doesn't work
      function selectAll() {
