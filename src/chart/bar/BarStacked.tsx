...
function CreateBarStacked(element: React.RefObject<SVGElement>, props: ChartStyle) {
	...
	newData.forEach((g, i) => {
		g.value.map((_, k) => {
			if (i >= 1) newData[i].value[k].stackedY += newData[i - 1].value[k].stackedY;
		});
	});

	if (xAxis.domain === undefined) xAxis.domain = x.filter((d) => d !== '');
	if (yAxis.domain === undefined) {
		const domain2 = d3.max(newData, (obj) => d3.max(obj.value, (obj) => obj.stackedY));
	...
	if (xAxis.enabled) {
		createXAxis(svg, xAxisType, props, false, barWidth * rowKeys.length * x.length, x);
	}
	...
	const onHover = (event: unknown, index: number) => {
		newData.forEach((data, i) => {
			if (i === index) {
				...
				const newYScale = d3.scaleLinear([0, newDomain !== undefined ? newDomain * 1.1 : 0], yAxis.range);
