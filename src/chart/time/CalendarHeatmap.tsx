...
function RemoveCalendarHeatmap(element: React.MutableRefObject<null>) {
  d3.select(element.current).selectAll('g').remove();
}

function CreateCalendarHeatmap(element: React.MutableRefObject<null>, props: CalendarHeatmapProps) {
  let {
    data,
    mapFunction,
    utcParse,
  ...
  */
  ...
  const cellSize = (width - margin.top - margin.right) / 53;
