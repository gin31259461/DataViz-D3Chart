...
function D3WordCloud(
  ...
  const wordScale = d3.scaleLinear().range(wordSizeRange).domain(wordSizeDomain);
  const colorScale = d3.scaleLinear().range(wordColorRange).domain(wordColorDomain);
  const layout = cloud()
    .size([width, height])
    .words(wordsData)
    .rotate(rotate)
